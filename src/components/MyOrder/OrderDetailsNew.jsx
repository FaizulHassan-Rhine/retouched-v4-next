...
const OrderDetailsNew = () => {
  ...
  const downloadFunc = (imageType) => {
    ...
    fetch(
      getApiBasicUrl +
      `/api/2023-02/get-order-image-detail-by-order-master-image-id?order_master_iamge_id=${orderData.id}`,
      {
        headers: {
      ...
      .then((data) => {
        ...
        data.status_code == 200 &&
          downloadAllFileWithIframe(data.results.order_image_detial, imageType);
      })
      .catch((error) => console.error("Failed to fetch order details:", error));

    setIsOpen(false);
  };
  const reworkedDownloadFunc = (imgType) => {
 
    fetch(
      `${getApiBasicUrl}/api/2024-05/get-rework-order-image-detail-by-order-master-id?order_master_id=${orderData.id}`,
      {
        headers: {
          Authorization: "bearer " + getToken,
          "Content-Type": "application/x-www-form-urlencoded",
      ...
      .then((data) => {
        ...
        data.status_code == 200 &&
          downloadAllFileWithIframeReworked(
            data.results.order_image_detial,
            imgType
          );
      })
      .catch((error) => console.error("Failed to fetch order details:", error));
      ...
      <div className="bg-[#FAFAFA] h-screen">
        ...
        <div className="">
          <div className="container mx-auto pt-2 relative">
            <div className=" absolute top-2 right-0 bg-gray-300 rounded-full">

            </div>
            <div className="flex justify-center items-center gap-3 mb-4">
              <h2 className="text-2xl text-center   text-green-900 font-bold">
                <i className="fa-solid mr-2 -mt-1 fa-basket-shopping"></i>ORDER DETAILS
            ...
            <div className="grid lg:grid-cols-3 justify-items-center items-start gap-5 md:gap-20 xl:gap-2">
              <div className="">
                {/* downloadable starte */}
                {
                  allData.some((item) => (item.status_id == 30 || item.status_id == 45) && item.is_rework == false) &&
                  ...
                  <div>
                    ...
                    <div className={`bg-white p-2 w-[230px] xl:w-[270px] rounded-lg shadow-lg overflow-y-auto  ${allData.some((item) => (item.status_id == 25 || item.status_id == 45)  && item.is_rework === true)  ? "h-[190px]" : "h-[130px] lg:h-[380px]"} `}>
                ...
                {
                  allData.some((item) => (item.status_id == 25 || item.status_id == 45)  && item.is_rework === true) &&
                  <div>
                    ...
                    <div className={`p-2 w-[230px] xl:w-[270px]  bg-white rounded-lg shadow-lg  overflow-y-auto  ${  allData.some((item) => (item.status_id == 30 || item.status_id == 45) && item.is_rework == false) ? "h-[150px]" :"h-[380px]" }`}>
              ...
              {allData.some((data) => data.status_id === 5) &&
                <div className="">
                  ...
                  <div className="pt-5 ml-16 w-full  relative">
                    {getCountsRework > 0 && (
                      <div
