...
const UploadImageOld = () => {
  ...
  const [visibleSection, setVisibleSection] = useState("25b49f1d-b72e-4d2e-8ded-7ac98fe3184e"); // New state to control visibility of sections
  ...
  const generateBgFirstCall = () => {
    ...
    if (getAiPromt.length == 0) {
      ...
      setLoadingImages(newLoadingArray);
      setImageLoader(true);
  ...
  const newOrderCreate = (newFile) => {
    ...
    fetch(getApiBasicUrl + "/api/2023-02/order-master-info", {
      ...
      .then((data) => {
        ...
        countImageCallIndex += (fileSlice.length - 1);
...
{console.log(data)}
        if (data.status_code == 200) {
          ...
          countImageCallIndex += 1;
          setStoreData(updateFileData);
          setProccessImgIndex((getProccessImgIndex) => getProccessImgIndex + 1);
          storeData.length > countImageCallIndex && dataTransfer(countImageCallIndex);
          break; // Break out of the loop if successful
        } else {
          retryCount++; // Increment retry count if status_code is not 200
          await delay(1000); // Wait for 1 second before retrying
  ...
  const dataTransfer = async (index) => {
    ...
    formData.append("order_master_id", imageOrderId);
        ...
        <div className="container mx-auto h-full pb-0">
          <div className="lg:flex justify-center pt-10 md:pt-20 gap-12">
            ...
            <div className="h-[450px] w-[370px] rounded-lg shadow-2xl relative hidden lg:block">
              ...
              <div className="absolute  bottom-[20px] left-[50%]" style={{ transform: "translateX(-50%)" }}>
            ...
            <div>
              <div className=" flex flex-col justify-items-center items-center ">
                  ...
                  {storeData.length > 0 &&
                    storeData.map((data, index) => (
                      <>
                          ...
                          >
                            <AfterBeforeImageStore proccesImages={data} counte={prCount} visibleSection={visibleSection} actionData={getActionData} />
                          </div>
                      </>
                    ))}
                  <button
                ...
                <div className="flex justify-center gap-4 shadow-lg w-full">
                  ...
                  {/* 
                  <button
                    onClick={() => handleServices("25b49f1d-b72e-4d2e-8ded-7ac98fe3194e")}
                  ...
                  {/* <button
                    onClick={() => handleServices("81206d6f-e2e5-46e8-836e-b657ab504392")}
                    className={`text-sm font-semibold py-1 px-2 flex flex-col justify-center items-center rounded ${activeSection === "81206d6f-e2e5-46e8-836e-b657ab504392"
                      ? "text-[#FFB752]"
                      : "text-black"
                      }`}
                  >
            ...
            <div className="h-full pb-5 lg:h-[450px] relative w-full lg:w-[400px] xl:w-[370px]  lg:shadow-2xl  flex flex-col  gap-3 md:gap-7 justify-start items-center  ">
              ...
              <div className='absolute top-0 left-0'>
                <h1 className='text-xs bg-[#0cab9f] text-white px-2 py-1 rounded-r-3xl font-semibold'>beta</h1>
              </div>
                  {/* Prompt Text Section with Input Field */}
                  <div className="p-4 mt-5 flex flex-col">
                    <span className='text-xs text-gray-500 '>Char: {getAiPromt.length}/100</span>

                    <textarea
                      onChange={(e) => setAiPromt(() => e.target.value)}
                      maxLength={100}
                      className={`w-full mt-1 resize-none text-black  shadow-lg focus:outline-none  text-sm h-20 box-border p-3 overflow-auto  rounded-tl-lg rounded-tr-lg border-solid border-[1px] 
                    focus-visible:border-[#0cab9f] focus-visible:outline-none 
                    ${imageLoader ? "bg-[#eeeeee] border-gray-500" : "bg-[#e8fdfc] border-[#0cab9f]"}`}
                      placeholder="Generate AI Background..."
                      rows="4"
                    ></textarea>
                    <button disabled={imageLoader} onClick={generateBgFirstCall} className={`${imageLoader ? "bg-gray-500 cursor-default" : "bg-[#0cab9f] cursor-pointer"} w-full p-1 text-center text-white`}>Generate</button>
                  </div>

                  {/* Image Grid */}
                  <div className="grid grid-cols-3 lg:grid-cols-3 xl:grid-cols-3 gap-4 px-4 pb-4 pt-2 h-[131px]">

                    {
                      loadingImages.map((item, index) => (
                        gnImageList[index] ?
                          <img
                            key={index}
                            onClick={() => setActionData(gnImageList[index])}
                            src={gnImageList[index]}
                            alt="prompt generated image"
                            className={` h-[110px] rounded shadow-md cursor-pointer transition-all hover:border-[3px] hover:border-green-700 
                                  ${gnImageList[index] == getActionData ? "border-[3px] border-green-700" : ""}`}
                          />
                          :
                          <div className='relative flex flex-col justify-center items-center bg-slate-100 shadow-lg'>
                            <div className="absolute top-0 right-0 w-full h-full bg-gray-800 opacity-85 animate-pulse" />
                          </div>
                      ))
                    }
                ...
                <div className="static lg:absolute top-0 left-0 w-full h-full bg-white z-50">
                  ...
                  <div className="p-4 mt-6 ">
                    <div>
                      ...
                      {/* Image Sections */}
                      {showFirstGrid ? (
                        <div className="grid grid-cols-4 lg:grid-cols-4 xl:grid-cols-5 gap-4 mt-3">
                          {/* custom image puload */}
...

                          {
                            presetRowImgList.map((item, index) =>
                              <img
                                ...
                                src={item}
                                alt={"Image " + index}
                                className={`w-[50px] h-[50px] rounded shadow-md cursor-pointer transition-all hover:border-[3px] hover:border-green-700 
                                  ${item == getActionData ? "border-[3px] border-green-700" : ""}
                                  `}
                              />
                            )
                          }
                        ...
                        <div className="grid  grid-cols-4 lg:grid-cols-4 xl:grid-cols-5 gap-4 mt-3">
                          ...
                          <div
                            ...
                            className="cursor-pointer w-[50px] h-[50px] rounded shadow-md text-xl flex flex-col items-center justify-center">

                            <span><IoMdColorFilter /></span>

                            <input onChange={handleColorChange} className='opacity-0 h-0 w-0' id="colorPick" name="color" type="color" />
                          </div>
                          {
                            colorList.map((color, index) => (
                              <div
                                ...
                                key={index}
                                className={`cursor-pointer w-[50px] h-[50px] rounded shadow-md transition-all hover:border-[3px] hover:border-green-700
                                   ${color === getActionData ? "border-[3px] border-green-700" : ""}`}
                ...
                <div className="static lg:absolute top-0 left-0 w-full h-full bg-white z-50">
                  ...
                  <div className="my-10">
                    <div className="grid grid-cols-2 justify-items-center gap-4 px-6">
                      <div onClick={() => setResizeOption('shopify')} className={`w-[135px] lg:w-auto xl:w-full border border-gray-300 px-3 flex flex-col items-center cursor-pointer hover:bg-blue-100 rounded-md shadow-md ${resizOption == 'shopify' && "bg-blue-200"}`}>
                        {/* <h1 className="font-semibold ">Shopify</h1> */}
                        <img src="/images/images/shopify.png" />
                        <h2 className=" text-sm -mt-2">2048 x 2048</h2>
                      </div>
                      <div onClick={() => setResizeOption('facebook')} className={`w-[135px] lg:w-auto xl:w-full border border-gray-300 px-3 flex flex-col items-center cursor-pointer hover:bg-blue-100 rounded-md shadow-md ${resizOption == 'facebook' && "bg-blue-200"}`}>
                        {/* <h1 className="font-semibold">Facebook</h1> */}
                        <img src="/images/images/facebook.png" />
                        <h2 className="text-sm -mt-2">1200 x 1200</h2>
                      </div>
                      <div onClick={() => setResizeOption('amazon')} className={`w-[135px] lg:w-auto xl:w-full border border-gray-300 px-3 flex flex-col items-center cursor-pointer hover:bg-blue-100 rounded-md shadow-md ${resizOption == 'amazon' && "bg-blue-200"}`}>
                        {/* <h1 className="font-semibold">Amazon</h1> */}
                        <img src="/images/images/amazon.png" />
                        <h2 className="text-sm -mt-2">2000 x 2000</h2>
                      </div>
                      <div onClick={() => setResizeOption('custom')} className={`w-[135px] lg:w-auto xl:w-full border cursor-pointer border-gray-300  flex flex-col items-center  hover:bg-blue-100 rounded-md shadow-md  ${resizOption == 'custom' && "bg-blue-200"}`}>
              ...
              {visibleSection === "25b49f1d-b72e-4d2e-8ded-7ac98fe3184e" && (
                <div className="static lg:absolute top-0 left-0 w-full h-full bg-white z-50">
                  ...
                  <div className="px-10  pt-10 pb-3 grid grid-cols-4 gap-3 justify-items-center  ">
                    ...
                    {
                      countRework == 0 && <p className='hidden lg:block absolute top-1/2 -translate-y-1/2 text-red-800 text-xl text-center'>No image selected for <b>Pro-touch</b></p>
                    }
