...
const UploadPageTestRhine = () => {
  ...
  const currentImages =
    storeData.length > 0 && storeData.slice(indexOfFirstItem, indexOfLastItem);

  const [isModalOpen, setModalOpen] = useState(false);
  // const [isReworkModalOpen, setReworkModalOpen] = useState(false);
  const [isReworkConfirmModalOpen, setIsReworkConfirmModalOpen] =
    useState(false);
  ...
  const updateOrderImageService = async (imgId, srvId) => {
    ...
    try {
      const response = await fetch(
        getApiBasicUrl + "/api/2023-02/order-image-service-insert",
        {
          method: "POST",
          headers: {
  ...
  const handleRework = (bl, id) => {
    ...
    if (
      storeData &&
      storeData[getOpenImg] &&
      storeData[getOpenImg].proccessImage.output_urls
    ) {
      const imageId =
        storeData[getOpenImg].proccessImage.output_urls[0]
          .order_image_detail_id;
      const imageDetail = {
        order_image_detail_id: imageId,
        service_item_id: id,
  ...
  const handleServices = (id) => {
    ...
    id == "25b49f1d-b72e-4d2e-8ded-7ac98fe3184e" &&
  ...
  const dataTransferForUrl = async (formData, index) => {
    ...
    while (retryCount < maxRetries) {
      try {
        const response = await fetch(`${getModelBaseUrl}process-image-url`, {
          method: "POST",
          headers: {
            Authorization: "bearer " + getToken,
          },
  ...
  const resizeFromServer = async (
    ...
    if (resizeImageCount == 0) {
      ...
      showToastMessage(
        "Image resizing is complete! Your file is now ready for download"
      );
  ...
  const reworkTransfer = () => {
    ...
    fetch(getApiBasicUrl + "/api/2024-04/user-rework-confirmation", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: "bearer " + getToken,
    ...
    fetch(
      ...
      .then((data) => {
        ...
        if (
          ...
          data.results.order_detail_charge_breakdown.length > 0
        ) {
          const imagePoint = parseInt(
            data.results.order_detail_charge_breakdown[0].point
          );
  ...
  useEffect(() => {
    getProccessImgIndex > 0 &&
      getProccessImgIndex == getTotalImage &&
      enalbleAllAnchorLink();
  }, [getProccessImgIndex]);
  ...
  return (
    <>
        ...
        <div className="container mx-auto h-full pb-0">
          <div className="lg:flex justify-center pt-10 md:pt-20 gap-12">
            ...
            <div className="h-[450px] w-[370px] rounded-lg shadow-2xl relative hidden lg:block">
              <div className="px-10  pt-10 pb-3 grid grid-cols-4 gap-4 justify-items-center  ">
                {currentImages.map((data, index) => (
                  <div
                    ...
                    key={index}
                    className={` h-[52px] w-[52px] hover:scale-105 duration-300 shadow-lg flex justify-center items-center rounded-lg cursor-pointer relative overflow-hidden border-gray-200 border ${
                      data.rework == true
                        ? "hidden"
                        : data.rework == null
                        ? "hidden"
                ...
                <div className="flex flex-col  md:flex-col justify-items-center justify-center items-center gap-4 pt-3">
                  ...
                  <div className="flex  gap-2 lg:flex-col ">
                    <button
                      ...
                      className={` px-12 py-2  text-white text-sm font-semibold  shadow-lg  rounded-3xl ${
                        getProccessImgIndex == 0
                          ? "bg-slate-300"
                          : "bg-[#F9A431]"
                      }`}
                    ...
                    getService.results.service_items.map((data, index) => (
                      <>
                        {data.is_visible === true && (
                          <div className="flex justify-center  lg:justify-center  ml-0 lg:ml-2 mt-2 mb-2 lg:mt-0 items-center gap-2 "></div>
                        )}
                      </>
                    ))}
                </div>
                      ...
                      <>
                        {data.is_visible === true && (
                          <button
                            disabled={
                              getTotalImage !==
                              proccessFailCount + getProccessImgIndex
                            }
                            onClick={() => handleServices(data.id)}
                            className={`text-sm font-semibold py-1 px-2 flex flex-col justify-center items-center rounded ${
                ...
                <div className="static lg:absolute top-0 left-0 w-full h-full bg-white z-50">
                  ...
                  <div className="my-10">
                    <div className="grid grid-cols-2 justify-items-center gap-4 px-6">
                      <div
                        onClick={() => setResizeOption("shopify")}
                        className={`border border-gray-300 px-3 flex flex-col items-center cursor-pointer hover:bg-blue-100 rounded-md shadow-md ${
                          resizOption == "shopify" && "bg-blue-200"
                        }`}
                      >
                      ...
                      <div
                        onClick={() => setResizeOption("facebook")}
                        className={` border border-gray-300 px-3 flex flex-col items-center cursor-pointer hover:bg-blue-100 rounded-md shadow-md ${
                          resizOption == "facebook" && "bg-blue-200"
                        }`}
                      >
                      ...
                      <div
                        onClick={() => setResizeOption("amazon")}
                        className={`border border-gray-300 px-3 flex flex-col items-center cursor-pointer hover:bg-blue-100 rounded-md shadow-md ${
                          resizOption == "amazon" && "bg-blue-200"
                        }`}
                      >
                      ...
                      <div
                        onClick={() => setResizeOption("custom")}
                        className={`border cursor-pointer border-gray-300  flex flex-col items-center  hover:bg-blue-100 rounded-md shadow-md  ${
                          resizOption == "custom" && "bg-blue-200"
                        }`}
                      >
                        ...
                        <div className="flex  gap-1 lg:p-1 xl:p-4">
                          <div className="flex flex-col ">
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                setResize((dt) => {
                                  return { ...dt, height: e.target.value };
                                });
                              }}
                            />
                          </div>
                          ...
                          <div className="flex flex-col">
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                setResize((dt) => {
                                  return { ...dt, width: e.target.value };
                                });
                              }}
                            />
                          </div>
                  ...
                  <div className="px-10  pt-10 pb-3 grid grid-cols-4 gap-3 justify-items-center  ">
                    {storeData.map((data, index) => (
                      <div
                        onClick={() => viewImage(index)}
                        key={index}
                        className={`h-[52px] w-[52px] flex items-center hover:scale-105 duration-300 shadow-lg rounded-lg cursor-pointer relative overflow-hidden border-gray-200 border ${
                      ...
                      >
                        ...
                        />
                        {data.proccessImage &&
                        data.proccessImage.output_urls ? (
                          ""
                        ) : (
              ...
              <div className="flex flex-col w-full p-2 shadow-3xl  md:flex-col justify-items-center justify-center items-center gap-4 lg:hidden">
                ...
                <div className="flex  gap-2 lg:flex-col ">
                  <button
                    disabled={getProccessImgIndex == 0}
                    onClick={downloadLatestFunc}
                    className={` px-12 py-2  text-white font-semibold  shadow-lg  rounded-3xl ${
                      getProccessImgIndex == 0 ? "bg-slate-300" : "bg-[#F9A431]"
                    }`}
                  >
                    Download
                  </button>
      ...
      {popupInfo.show && (
        <PopUpInfo
          msg={popupInfo.message}
          dark={true}
          callBackPopupClose={callBackPopupClose}
        />
      )}
...
export default UploadPageTestRhine;
