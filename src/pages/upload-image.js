...
useEffect(() => {
    ...
    if (stored) {
      try {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          // Trust the session data and use it directly
          setFileInfo(parsed);         // ✅ Set context
          setStoreData(parsed);        // ✅ Set for local processing

          console.log("✅ setFileInfo & setStoreData triggered with:", parsed);
  ...
  if (storeData.length > 0 && runOnceTime === false) {
    const timer = setTimeout(() => {
      uploadFile();
    }, 100); // Add a slight delay to allow React state to settle
